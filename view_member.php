...
<?php
// Function to get trainer name by email
function getTrainerNameByEmail($member_id, $conn)
{
    if (empty($member_id)) {
        return 'N/A';
    }

    $trainerQuery = $conn->query("SELECT t.name FROM trainers t 
                                 INNER JOIN registration_info r ON t.id = r.trainer_id
                                 WHERE r.member_id = '$member_id'");
    $trainer = $trainerQuery->fetch_assoc();
    return $trainer ? ucwords($trainer['name']) : 'N/A';
}
?>
...
<div class="container-fluid">
	<div class="row">
		<div class="col-md-4">
			...
			<p>Trainer: <b><?php echo getTrainerNameByEmail($member_id, $conn); ?></b></p>
